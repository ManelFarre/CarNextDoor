...
// Comprova si l'usuari està loguejat
    ...
    function updateButtons() {
        prevBtn.style.display = carCarousel.scrollLeft > 0 ? "block" : "none";
        nextBtn.style.display = carCarousel.scrollLeft + carCarousel.clientWidth < carCarousel.scrollWidth ? "block" : "none";
    }

    carCarousel.addEventListener("scroll", updateButtons);
    updateButtons(); // Llamada inicial para esconder botones si es necesario
    ...
    rentButtons.forEach(function (button) {
        button.addEventListener("click", function (event) {
            event.stopPropagation(); // Evita que otros eventos en el contenedor se activen

            // Buscar la tarjeta de coche más cercana al botón
            const carCard = this.closest(".car-card");

            // Obtener información relevante del coche
            const carData = {
                name: carCard.querySelector("h3").textContent,
                description: carCard.querySelector("p").textContent,
                price: carCard.querySelector("strong").textContent,
